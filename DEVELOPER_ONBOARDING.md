...
1. Изучить схему БД в `apps/server/prisma/schema.prisma`
2. Посмотреть примеры компонентов в `packages/ui/src/components/`
3. Изучить доменную архитектуру в `apps/server/src/auth/index.ts`
4. Запустить приложение и посмотреть как всё взаимодействует
5. Изучить API клиента в `packages/api/src/client/`

## Контакты и поддержка

При возникновении вопросов обращайтесь к ведущему разработчику или изучите:
- Правила архитектуры в `.cursor/rules/`
